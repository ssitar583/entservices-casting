...
# Testcase ID : TCID011
# Testcase Description : Requests the active source in the network., verify the output response
import subprocess
import json
import requests
import time
...
print("TC Description - Hit the curl command for getActiveSourceStatus method and verify that status is obtained as true in output response")
...
#ToDo - send message emulation for onActiveSourceChange	Triggered with the active source device changes.

curl_response = Utils.send_curl_command(HdmiCecSinkApis.request_active_source)
if curl_response:
...
tc_id = 'TCID011_requestActiveSource'
